{{--     
    <span role="img" class="anticon "><svg
            width="713"
            height="77"
            viewBox="0 0 713 77"
            fill="none"
            ...
            <path
              ...
              fill="url(#paint0_linear_204_295)"
              fill-opacity="0.8"></path>
            <defs>
              <linearGradient
                id="paint0_linear_204_295"
                x1="186.725"
                y1="20.6343"
                x2="604.321"
                y2="257.749"
...
function updateURL(slug = "", type = "chapter") {
  let newURL = slug ? `${projectFolder}/${type}/${encodeURIComponent(slug)}` : `${projectFolder}/`;
...
function setLayout(state = "all") {
  if (state === "all") {
    ...
    chapterDetailsCard.classList.remove("text-start");
    chapterDetailsCard.classList.add("text-center");

  } else if (state === "chapter") {
    chapterDetailsCol.className = "col-12";
    chapterDetailsCard.classList.remove("text-center");
    chapterDetailsCard.classList.add("text-start");
...
function renderBackButton() {
  ...
  backBtnWrapper.id = "backToChaptersWrapper";
  backBtnWrapper.className = "mt-4 text-center";
  backBtnWrapper.innerHTML = `
    <button id="backToChapters" class="btn" 
      style="background: linear-gradient(94deg, rgb(162, 218, 0) 26.2%, rgb(147, 199, 104) 99.21%); color: white; border: none; padding: 10px 20px; border-radius: 8px;">
      ⬅ Back to Chapters
    </button>
  `;

  // ✅ Place button after the second row (bottom of page content)
  document.getElementById("secondRow").after(backBtnWrapper);
...
// Show Words
function showWords(chapter) {
  setLayout("chapter");
  wordsDiv.innerHTML = "";

  let hasWords = false;
  for (const word in data[chapter]) {
    if (word === "chapter_details" || word === "chapter_slug") continue;
    hasWords = true;
    ...
    btn.onclick = () => {
      ...
      const slug = data[chapter][word].word_slug || word;
      updateURL(slug, "word");
    };
    wordsDiv.appendChild(btn);
  }

  if (!hasWords) wordsDiv.innerHTML = `<div class="alert alert-info m-0">No words found in this chapter.</div>`;

  fadeIn(wordsCard);
  fadeIn(chapterDetailsCard);
  renderBackButton();
}

// Show Dreams
function showDreams(chapter, word) {
  setLayout("word");
  dreamsDiv.innerHTML = "";

  let hasDreams = false;
  for (const dreamKey of Object.keys(data[chapter][word] || {})) {
    if (dreamKey === "word_slug") continue;
    hasDreams = true;
    const btn = document.createElement("button");
    ...
    btn.onclick = () => {
      ...
      const slug = data[chapter][word][dreamKey].dream_slug || dreamKey;
      updateURL(slug, "dream");
    };
    dreamsDiv.appendChild(btn);
  }

  if (!hasDreams) dreamsDiv.innerHTML = `<div class="alert alert-info m-0">No dreams found for this word.</div>`;
...
function showDescription(chapter, word, dream) {
  const meaning = (data[chapter][word][dream] && data[chapter][word][dream].meaning) || "<em>No description available.</em>";

  descriptionDiv.innerHTML = `
    <div class=" p-3 mb-3 bg-white rounded" style="border:1.4px solid #4ea72e">
      <p style="font-size:14px;"><strong> Chapter:</strong> ${chapter}</p>
      <p style="font-size:14px;><strong> Word:</strong> ${word}</p>
      <p style="font-size:14px;><strong> Dream:</strong> ${dream}</p>
      <p style="font-size:14px;><strong> Description:</strong> ${meaning}</p>
    </div>`;
...
function showAllChapterDetails() {
  ...
  chapterDetailsDiv.innerHTML = Object.keys(data)
    .map(chapter => `
      <div class="chapter-item mb-3 p-3 rounded" style="cursor:pointer; border: 1.4px solid #4ea72e" data-chapter="${chapter}">
        <div class="d-flex align-items-start">
          <h5 class="me-3"> ${chapter}: &nbsp </h5>
          <p class="mb-0">${data[chapter].chapter_details || "<em>No details available.</em>"}</p>
        </div>
      </div>
    `).join("");
  ...
  document.querySelectorAll(".chapter-item").forEach(item => {
    item.addEventListener("click", () => {
      ...
      selectedWord = selectedDream = null;

      chapterDetailsDiv.innerHTML = `
        <div class="p-3 rounded" style="border: 1.4px solid #4ea72e;">
          <div class="d-flex align-items-start">
            <h5 class="me-2"> ${chapter}: &nbsp</h5>
            <p class="mb-0">${data[chapter].chapter_details || "<em>No details available.</em>"}</p>
          </div>
        </div>
      `;
      ...
      showWords(chapter);
      updateURL(data[chapter].chapter_slug || chapter, "chapter");
    });
  });
...
document.addEventListener("DOMContentLoaded", () => {
  fetch(`${projectFolder}/index.php?action=data`)
    .then(res => res.json())
    .then(json => {
      data = json;
      showAllChapterDetails();
    ...
    .catch(err => {
      chapterDetailsDiv.innerHTML = "<div class='text-danger'>Error loading content.</div>";
